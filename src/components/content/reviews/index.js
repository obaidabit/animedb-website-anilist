...
export default function Reviews({ animeId }) {
  ...
  useEffect(() => {
    ...
    getReviewsAPI(animeId).then((result) => {
      if (mounted) {
        setData(result.data);
        ...
        data?.map((data) => (
          <div className="grid md:grid-cols-[1fr,5fr] lg:grid-cols-[1fr,8fr] gap-10 md:gap-5">
            ...
            <div className="relative px-5 pt-2 pb-4 border-2 border-light_secondary dark:border-dark_secondary rounded-2xl">
              <div className="w-16 -top-11 absolute overflow-hidden md:hidden inline-block">
                <div className="h-11 w-11 bg-white rotate-45 border-2 border-light_secondary dark:border-dark_secondary dark:bg-black transform origin-bottom-left"></div>
              </div>
              <div className="w-10 top-5 -left-10 absolute overflow-hidden hidden md:inline-block">
                <div className=" h-16 border-2 border-light_secondary dark:border-dark_secondary bg-white  dark:bg-black  -rotate-45 transform origin-top-right"></div>
              </div>
              <div className="flex flex-col py-2 md:flex-row">
                <h5 className="pr-2 text-2xl font-bold ">
                  {data?.user?.username}
                </h5>
                <p className="pt-1 text-lg text-gray-500 ">{data?.date}</p>
              </div>
              <p>{data?.review}</p>
