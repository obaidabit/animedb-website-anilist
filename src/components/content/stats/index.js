...
export default function Stats({ animeId }) {
  ...
  useEffect(() => {
    ...
    getStatAPI(animeId).then((result) => {
      if (mounted) {
        setData(result.data);
      ...
      {loading ? (
        ...
        <div className="flex flex-col justify-between w-full gap-10 md:gap-0 md:flex-row">
          <div className="flex flex-col w-full gap-2">
            <h1 className="mb-3 text-3xl font-bold">Summary Stats</h1>
            ...
            <p className="text-xl font-bold">
              On-Hold :{" "}
              <span className="font-normal">
                {data?.on_hold.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
              </span>
            </p>
            <p className="text-xl font-bold">
              Dropped :{" "}
              <span className="font-normal">
                {data?.dropped.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
              </span>
            </p>
            <p className="text-xl font-bold">
              Plan to Watch :{" "}
          ...
          <div className="flex flex-col gap-2">
            <h1 className="mb-3 text-3xl font-bold">Scored Stats</h1>
